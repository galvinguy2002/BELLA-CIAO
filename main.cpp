...
datamodule() : city{"New Delhi", "Madrid", "Washington DC", "Chicago", "Capetown",
                    "Singapore", "Dubia", "Kathmandu", "Sydney", "London", "Nottingome", "Leads", "Paris", "Rome", "Berlin"} {}
...
void costdeclaration()
{
    ...
    // The cost of travelling between any two cities in direct flight is initialized.
    // In further steps, the minimum fare will be calculated
    ...
    // the main array i.e. arr is not changed. Instead, the values are copied to
    // structure and then the structure store with data member cost is changed. The structure store will hold the
...
void algorithmic_implementation()
{
    for (int src = 0; src < N; src++)
    {
        ...
        bool sptSet[N];
        for (int i = 0; i < N; i++)
            dist[i] = infinity, sptSet[i] = false, parent[i] = i;
        dist[src] = 0;
        for (int count = 0; count < N - 1; count++)
        {
            ...
            for (int v = 0; v < N; v++)
            {
                if (!sptSet[v] && ini[u].cost[v] != infinity && dist[u] != infinity && dist[u] + ini[u].cost[v] < dist[v])
                {
                    dist[v] = dist[u] + ini[u].cost[v];
                    parent[v] = u;
                    travel[src].cost[v] = dist[v];
                    travel[src].array[v] = u;
                }
            }
        }
    }
...
void passengerdetailedticket(int no, string firstname[], string lastname[], string sex[], int age[])
{
    ...
    int main()
    {
        ...
        dijkstraalgorithm ob2;
        ob1.algorithmic_implementation();
        // ob2.algorithmic_implementation();
        string userchoice[2];
